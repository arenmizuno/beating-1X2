...
def compare_to_market(predictions: pd.DataFrame) -> pd.DataFrame:
    """Model vs market on every (track, model, fold), on identical rows."""
    rows = []
...
def write_summary(comparison: pd.DataFrame, bets: pd.DataFrame) -> None:
    ...
    wf_mean = (
        walk_forward.groupby(["track", "model"])[
            ["model_log_loss", "market_log_loss", "model_brier", "model_ece", "model_accuracy"]
        ]
        .mean()
        .reset_index()
    )
    ...
    lines = [
        "# Value-Bet Detector -- Results Summary",
        "",
        ...
        "",
        holdout[["track", "model", "n", "model_log_loss", "market_log_loss", "logloss_gap"]]
        .round(4)
